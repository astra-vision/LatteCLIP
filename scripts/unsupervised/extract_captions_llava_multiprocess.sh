...
echo "CLIP_PREDICTION_PATH: ${CLIP_PREDICTION_PATH}"
...
do
  ...
  do
    ...
    CUDA_VISIBLE_DEVICES=${gpu_id} python preprocess/extract_captions_llava_1_6.py -p ${current_process_id} \
      -w ${WORLD_SIZE} -od ${OUTPUT_DIR} \
      --split train \
      --clip-prediction-path ${CLIP_PREDICTION_PATH} \
