...
@click.command()
@click.option("--dataset_dir", default="$LATTECLIP_DATA_DIR/ucf101")
@click.option("--preprocess_dir", default="$LATTECLIP_DATA_DIR/ucf101_preprocess")
...
Convert caltech-101 dataset to WebDataset format using the image ID - product title mapping. 
    ...
    with open(split_path, "r") as f:
        ...
        for key in split_data.keys():
            for i in range(len(split_data[key])):
                split_data[key][i][2] = split_data[key][i][2].replace(".", " ")
    ...
    for split, data in zip(["train", "val"], [data_train, data_test]):
        ...
        for image_path, _, class_name in tqdm(data):
            ...
            # Copy image
            src_image_path = os.path.join(dataset_dir, "UCF-101-midframes", image_path)
