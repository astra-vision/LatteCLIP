...
@click.command()
@click.option("--dataset_dir", default="$LATTECLIP_DATA_DIR/fgvc_aircraft")
@click.option("--preprocess_dir", default="$LATTECLIP_DATA_DIR/fgvc_aircraft_preprocess")
...
Convert caltech-101 dataset to WebDataset format using the image ID - product title mapping. 
    ...
    classnames = []
    with open(os.path.join(dataset_dir, "variants.txt"), "r") as f:
        lines = f.readlines()
        for line in lines:
            classnames.append(line.strip())
    cname2lab = {c: i for i, c in enumerate(classnames)}

    train = read_data(dataset_dir, cname2lab, "images_variant_train.txt")
    val = read_data(dataset_dir, cname2lab, "images_variant_val.txt")
    test = read_data(dataset_dir, cname2lab, "images_variant_test.txt")
    ...
    for split, data in zip(["train", "val"], [data_train, data_test]):
        ...
        for image_path, _, class_name in tqdm(data):
            ...
            # Copy image
            src_image_path = os.path.join(dataset_dir, "images", image_path)
...
def read_data(dataset_dir, cname2lab, split_file):
        ...
        with open(filepath, "r") as f:
            ...
            for line in lines:
                ...
                impath = os.path.join(dataset_dir, "images", imname)
