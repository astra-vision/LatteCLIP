...
@click.command()
@click.option("--dataset_dir", default="$LATTECLIP_DATA_DIR/oxford_flowers")
@click.option("--preprocess_dir", default="$LATTECLIP_DATA_DIR/flower102_preprocess")
...
Convert caltech-101 dataset to WebDataset format using the image ID - product title mapping. 
    ...
    for split, data in zip(["train", "val"], [data_train, data_test]):
        ...
        for image_path, _, class_name in tqdm(data):
            ...
            # Copy image
            src_image_path = os.path.join(dataset_dir, "jpg", image_path)
